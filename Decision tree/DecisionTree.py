...
class DecisionTreeReg:
    def __init__(self, max_depth=3, criterin="entropy"):
        self.max_depth = max_depth
        self.tree = None
        self.criterion = criterin
...
class DecisionTreeCla:
    ...
    def entropy(self, y):
        ...
        return -np.sum(p * np.log2(p+0.00001))
    ...
    def info_gain(self, y, left_indices, right_indices):
        if self.criterion == "entropy":
            impurity_fun = self.entropy
        elif self.criterion == "gini":
            impurity_fun = self.gini
        parent_impurity = impurity_fun(y)
        left_impurity = impurity_fun(y[left_indices])
        right_impurity = impurity_fun(y[right_indices])

        w_left = len(left_indices)/len(y)
        w_right = len(right_indices)/len(y)
        information_gain = parent_impurity - left_impurity*w_left - right_impurity*w_right
    ...
    def fit(self, X, y, depth=0):
        ...
        for i in range(num_features):
            feature_value = X[:, i]
            uniqie_value = np.unique(feature_value)
            for j in uniqie_value:
                left_indices, right_indices = self.split_data(X, i, j)
                if len(left_indices) == 0 or len(right_indices) == 0:
                    continue
        ...
        self.tree = Node(feature=feature, value=value,
                         left=left_sub_tree, right=right_sub_tree)
        return self.tree
